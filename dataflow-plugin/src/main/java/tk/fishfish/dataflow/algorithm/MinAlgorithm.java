...
public class MinAlgorithm implements Algorithm {
    ...
    public void compute(Argument argument) {
        ...
        log.info("计算 {}-{} 最小值, 输出表: {}", inTable, col, outTable);
        ...
        try {
            ...
            if (count == 0) {
                ...
                min = rdd.reduce(Double::max);
            }
        } finally {
            rdd.unpersist();
        }
        log.info("计算 {}-{} 最小值: {}", inTable, col, min);

        // 结果注册为临时表
        MinRow row = new MinRow();
        row.setValue(min);
        spark.createDataFrame(Collections.singletonList(row), MinRow.class).toDF(col).createOrReplaceTempView(outTable);
    ...
    @Data
    public static class MinRow implements Serializable {
